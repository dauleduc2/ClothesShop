...
const Product: React.FunctionComponent<ProductProps> = ({ describe, imgUrl, price }) => {
    return (
        <Card className="duration-1000 transform w-44 sm:w-52 hover:-translate-y-1" key={describe}>
            <CardActionArea>
                <CardMedia component="img" image={imgUrl} alt="green iguana" />
            </CardActionArea>
            <CardContent>
                <p className="text-sm font-semibold text-left line-clamp-2">{describe}</p>
            </CardContent>
            <CardActions>
