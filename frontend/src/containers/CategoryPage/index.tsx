...
interface CategoryPageProps extends RouteComponentProps<RouteParams> {}

const CategoryPage: React.FunctionComponent<CategoryPageProps> = ({ match }) => {
    const [products, setProducts] = React.useState<ProductToShowDTO[]>([]);
    const { name } = match.params;
    React.useEffect(() => {
        async function getProductList() {
            const res = await productApi.getAllProductByType(name);
